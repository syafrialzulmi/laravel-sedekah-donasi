                      ...
                      <div class="row g-2">
                        @foreach($perms as $p)
                          @php
                            ...
                            $checked = old("permission.$p->id")
                                      || in_array($p->id, $rolePermissionIds ?? []);
                          @endphp
...
<style>
  ...
  .check-badge{
    ...
    display:flex; align-items:center; justify-content:center;
    background-color: var(--bs-primary); color:#fff; flex: 0 0 24px;
  }
  .form-check-input:checked + .perm-card .check-badge{ display:flex !important; }
  .min-w-0{ min-width:0; }
</style>
@endpush
